...
static int cb_iptables_rule(const struct filterent *ent, struct fg_misc *misc) {
  ...
  if (neednat)
    orules++, oprintf("-t nat -A %s%s %s %s%s\n", subchain, natchain,
...
int fg_iptables(struct filter *filter, int flags) {
  ...
  if (!(flags & FF_NOSKEL)) {
    if ((flags & FF_LSTATE) && (feat & (A_TCP | A_UDP))) {
      if (feat & A_TCP) {
        r += nchains;
        oputs("-I INPUT -p tcp ! --syn -m conntract --ctstate ESTABLISHED -j "
              "ACCEPT");
        oputs("-I OUTPUT -p tcp ! --syn -m conntract --ctstate ESTABLISHED -j "
              "ACCEPT");
        oputs("-I FORWARD -p tcp ! --syn -m conntract --ctstate ESTABLISHED -j "
              "ACCEPT");
      }
      if (feat & A_UDP) {
        r += nchains;
